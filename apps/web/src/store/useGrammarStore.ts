...
const markSingleError = (marker: Mark, error: GrammarError, options?: { selected?: boolean; activeIndex?: number }) => {
  ...
  marker.mark(keyword, {
    ...
    acrossElements: true,
    className: cn("grammar-error", error.type, options?.selected && options?.activeIndex !== undefined && `active-${options.activeIndex}`),
    filter: () => {
...
const getPreviewScrollContainer = (element: HTMLElement): HTMLElement | null => {
  const containers = Array.from(document.querySelectorAll<HTMLElement>('[data-preview-scroll-container="true"]'));
...
export const useGrammarStore = create<GrammarStore>((set, get) => ({
  ...
  checkGrammar: async (text: string) => {
    ...
    const apiKey =
      selectedModel === "doubao" ? doubaoApiKey : selectedModel === "openai" ? openaiApiKey : selectedModel === "gemini" ? geminiApiKey : deepseekApiKey;
    const modelId =
      selectedModel === "doubao" ? doubaoModelId : selectedModel === "openai" ? openaiModelId : selectedModel === "gemini" ? geminiModelId : deepseekModelId;

    set({ isChecking: true });

    try {
      const response = await fetch(apiUrl("/v1/resume/grammar"), {
        method: "POST",
        headers: {
